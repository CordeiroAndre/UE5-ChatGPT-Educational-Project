...
UCLASS()
class HISTORICALFIGURES_API AHFPlayerController : public APlayerController
{
	GENERATED_BODY()

	DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnStatusChange, const EUserStatus&, NewStatus );
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnNewRecognition, const FString&, Message);
...
public:
	...
	UPROPERTY(BlueprintAssignable, Category="TCC | Speech Recognition")
	FOnNewRecognition OnCapturedMic;
	 ...
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="TCC | Speech Recognition")
